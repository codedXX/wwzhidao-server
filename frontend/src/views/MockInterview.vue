...
<template>
  <div class="mx-auto max-w-4xl space-y-6 animate-fade-in">
    <div>
      ...
      <p class="mt-1 text-slate-500">选择面试类型，补充岗位背景后开始你的 AI 模拟面试。</p>
    </div>

    <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_280px]">
      <div class="glass-card space-y-5 p-6">
        <div>
          <label class="mb-3 block text-sm font-medium text-slate-700">面试类型</label>
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            >
              ...
              <p class="mt-1 text-xs text-slate-500">深度技术面试，约 12 道题</p>
            </button>
            <button
            ...
            >
              ...
              <p class="mt-1 text-xs text-slate-500">行为面试 + HR，约 8 道题</p>
            </button>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <label class="mb-1.5 block text-sm font-medium text-slate-700">候选人姓名</label>
            <input v-model="form.candidateName" class="input-field" placeholder="你的名字" />
          </div>
          <div>
            <label class="mb-1.5 block text-sm font-medium text-slate-700">目标公司</label>
        ...
        <div>
          <label class="mb-1.5 block text-sm font-medium text-slate-700">职位描述（JD）</label>
          <textarea v-model="form.jd" class="input-field min-h-[120px] resize-y" placeholder="粘贴职位描述。JD 和简历内容至少填写一项。"></textarea>
        </div>

        <div>
          <label class="mb-1.5 block text-sm font-medium text-slate-700">简历内容</label>
          <textarea v-model="form.resumeContent" class="input-field min-h-[120px] resize-y" placeholder="粘贴你的简历内容，AI 会生成更贴近你的问题。"></textarea>
        </div>

        <p v-if="error" class="rounded-lg bg-red-50 px-4 py-2 text-sm text-red-500">{{ error }}</p>

        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <p class="text-xs text-slate-400">开始前建议至少填写：岗位名称 + JD / 简历内容中的一项。</p>
          <button @click="handleStart" class="btn-primary min-w-[180px]" :disabled="!canStart">
            <span v-if="loading" class="animate-pulse-soft">⏳ 正在连接面试官...</span>
      ...
      <aside class="space-y-4">
        <div class="glass-card p-5">
          ...
          <p class="mt-2 text-sm leading-6 text-slate-600">{{ interviewSummary }}</p>
        </div>
        <div class="glass-card p-5">
